...
loop_break: // Hard exit from the above loop
      ...
      for (i = 0; i <= len; i++)
      {
          if ((testType == TEST_TYPE_LINE) && (breaksActual[i] == LINEBREAK_ALLOWBREAK))
          {
             /* The unicode test data doesn't differentiate between the two, so
              * neither should we. */
